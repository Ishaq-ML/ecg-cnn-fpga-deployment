module layer_conv3_k3 #(
    parameter WEIGHT_FILE = "hex/w_weight3.hex",
    parameter BIAS_FILE   = "hex/b_bias3.hex",
    ...
    reg signed [15:0] window  [0:2][0:31];
    reg signed [15:0] weights [0:6143];  // 64 filters × 3 × 32
    ...
    always @(*) begin
        ...
        for (m = 0; m < 3; m = m + 1)
            for (n = 0; n < 32; n = n + 1)
                sum = sum + (window[2 - m][n] * weights[FILTER_ID*96 + m*32 + n]);
    end
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            // FIX: same off-by-one fix as conv2
